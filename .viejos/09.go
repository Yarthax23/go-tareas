//09 Agregar time, CURRENT_TIME Arreglar func actualizar/editar por las nuevas columnas
// Modificar el Struct POST en Go
...
func handleEditarPost(w http.ResponseWriter, r *http.Request) {
	...
	if contenido, ok := datosParciales["contenido"]; ok {
		query += fmt.Sprintf("contenido = $%d, ", i)
		args = append(args, contenido)
		i++
	}

	if fechaStr, ok := datosParciales["fecha"]; ok {
		fecha, err := time.Parse(time.RFC3339, fechaStr)
		if err != nil {
			http.Error(w, "Formato de fecha inválido. Usa RFC3339 (ej: 2025-05-16T15:04:05Z)", http.StatusBadRequest)
			return
		}
	...
	if resuelto, ok := datosParciales["resuelto"]; ok {
		if resuelto != "NO" && resuelto != "SI" {
			http.Error(w, "Valor inválido para 'resuelto': debe ser 'SI' o 'NO'", http.StatusBadRequest)
			return
		}
		query += fmt.Sprintf("resuelto = $%d, ", i)
		args = append(args, resuelto)
		i++
	}

	if len(args) == 0 {
		http.Error(w, "No se proporcionó ningún campo para actualizar", http.StatusBadRequest)
		return
	}
	...
	fmt.Fprintf(w, "Tarea %d actualizada correctamente\n", intID)
...
func handleToggle(w http.ResponseWriter, r *http.Request) {
	...
	// Leer el valor actual de "resuelto"
	var actual string
	err = db.QueryRow("SELECT resuelto FROM posts WHERE id = $1", intID).Scan(&actual)
	if err != nil {
		http.Error(w, "No se encontró el post con ese ID", http.StatusNotFound)
		return
	}
	...
	_, err = db.Exec("UPDATE posts SET resuelto = $1 WHERE id = $2", nuevo, intID)
	if err != nil {
...
func handleVer(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query("SELECT id, contenido, resuelto, fecha FROM posts ORDER BY id ASC")
	if err != nil {
	...
	for rows.Next() {
		var id int
		var contenido, resuelto string
		var fecha sql.NullTime // puede ser artificial y poco fidedigno
		err := rows.Scan(&id, &contenido, &resuelto, &fecha)
		if err != nil {
		...
		if fecha.Valid {
			fmt.Fprintf(w, "[%d] %s %s %s\n", id, resuelto, fecha.Time.Format("2006-01-02 15:04"), contenido)
		} else {
			fmt.Fprintf(w, "[%d] %s %s\n", id, resuelto, contenido)
		}
...
func handleVerPorID(w http.ResponseWriter, r *http.Request) {
	...
	row := db.QueryRow("SELECT id, contenido, resuelto, fecha FROM posts WHERE id = $1", intID)
	err = row.Scan(&id, &contenido, &resuelto, &fecha)
	if err != nil {
		http.Error(w, "No se encontró el POST con ese ID", http.StatusNotFound)
		return
	}
